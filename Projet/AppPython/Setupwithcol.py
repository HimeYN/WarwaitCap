            ...
            if self.all == 0:
                ...
                for ligne in tqdm(lis):
                    ...
                    if cursor.fetchall().__sizeof__() == 0:
                        ...
                        insert_warwait = "UPDATE warwait SET grade = %s, site = %s, "+w_str+"reussite=%s, positionnement=%s, competences=%s, cv_code=%s, pe=%s, en_mission=%s WHERE nom = %s"

                        params = (grade,site) + only_weeks + (success_rate,company,skills,str(34),pe,"false",name)
                    try:
